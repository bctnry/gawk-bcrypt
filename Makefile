# 2025.12.14 (zlin): this is modified from the makefile of openwall's
# bcrypt. instead of producing multiple *.o, this produces one (1)
# bcrypt.so.
...
LD = $(CC)
RM = rm -f
CFLAGS = -W -Wall -Wbad-function-cast -Wcast-align -Wcast-qual -Wmissing-prototypes -Wstrict-prototypes -Wshadow -Wundef -Wpointer-arith -O2 -fomit-frame-pointer -funroll-loops -fPIC
ASFLAGS = -c
LDFLAGS = -s
...
all:
	make all_bcrypt
	gcc -fPIC -shared -c -O -g main.c
	gcc -o bcrypt.so -shared $(CRYPT_OBJS) main.o

all_bcrypt: $(CRYPT_OBJS) man
